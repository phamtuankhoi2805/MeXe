 ...
 * Bao gồm: lấy giỏ hàng, thêm sản phẩm, cập nhật số lượng, xóa sản phẩm, đồng
 * bộ giỏ hàng
 * hiết bị nào, giỏ hàng sẽ tự động đồng bộ
 * 
 * @author
 * @version
 */
...
public class CartController {
    ...
    public ResponseEntity<Map<String, Object>> addToCart(@RequestBody Map<String, Object> request) {
        ...
        try {
            ...
            Long colorId = request.get("colorId") != null
                    ? Long.valueOf(request.get("colorId").toString())
                    : null;
     ...
     * Cho phép client gửi danh sách sản phẩm từ thiết bị local để merge với giỏ
     * hàng trên server
     * 
     * Logic:
