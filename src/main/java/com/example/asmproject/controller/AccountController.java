...
public class AccountController {
    ...
    public String doiMatKhau(@RequestParam(required = false) String currentPassword,
        ...
        if (user.getProvider() != null && user.getProvider().equals("google")) {
            redirectAttributes.addFlashAttribute("error", "Tài khoản đăng nhập bằng Google không thể đổi mật khẩu. Vui lòng quản lý mật khẩu trên tài khoản Google của bạn.");
            return "redirect:/tai-khoan";
        }
    ...
    public String themDiaChi(@RequestParam(required = false) String fullName,
        ...
        try {
            ...
            Address address = addressService.addAddress(
                user.getId(), fullName.trim(), phoneDigits, province.trim(), district.trim(), ward.trim(), street.trim()
            );
